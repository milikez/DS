...
public:
    ...
    public:
         ...
         *
         * @return const_iterator 当前节点的迭代器.
         */
...
public:
    ...
    iterator erase(iterator from, iterator to)
    {
        ...
        if (from == to)
            return from; // 如果from和to相同，不进行任何操作

        // 连接from的前一个节点和to的后一个节点
        if (from.current->prev) {
        ...
        iterator itr = from;
        while (itr != to) {
            iterator tmp = itr; // 保存当前迭代器的位置
            delete tmp.current; // 删除当前节点
            --theSize;          // 更新容器大小
            ++itr;              // 更新迭代器
        }
